"""
Random Forest Predictor for Page Replacement
=============================================
sklearn RandomForestClassifier with proper ML Pipeline
...
class RandomForestPredictor:
    """
    Random Forest Classifier for Page Access Prediction
    Uses sklearn RandomForestClassifier with full ML pipeline
    """
    ...
    def load_data(self, sequences):
        ...
        for seq in sequences:
            ...
            for i in range(self.context_window, len(pages)):
                # Features: last N pages (encoded as numbers)
    ...
    def engineer_features(self, features, targets):
        ...
        columns += ['unique_count', 'max_freq', 'most_recent']
        
        X = pd.DataFrame(X_data, columns=columns)
        y = self.label_encoder.transform(targets)
    ...
    # ================================================
    def build_model(self, n_estimators=100, max_depth=10):
        """
        ...
        """
        print("Step 5: Building Model (RandomForestClassifier)...")
    ...
    def evaluate(self, X_test, y_test):
        ...
        feature_names = [f'page_{i}' for i in range(self.context_window)] + ['unique_count', 'max_freq', 'most_recent']
    ...
    def run_pipeline(self, sequences):
        ...
        """
        print("\n" + "="*50)
        print("RANDOM FOREST ML PIPELINE")
        print("="*50)
        ...
        if len(self.history) >= self.context_window and self.is_fitted:
            ...
            try:
                if all(p in self.classes_ for p in context):
                    ...
                    feature_row = list(encoded) + [len(unique), max(counts) if len(counts) > 0 else 0, encoded[-1]]
                    X = self.scaler.transform([feature_row])
                    pred_idx = self.model.predict(X)[0]
                    self.last_prediction = self.label_encoder.inverse_transform([pred_idx])[0]
                else:
                    self.last_prediction = None
            except:
                self.last_prediction = None
                ...
                if all(p in self.classes_ for p in context):
                    ...
                    for i in range(self.frame_count):
                        ...
                        if page and page in self.classes_:
                            page_idx = list(self.model.classes_).index(self.label_encoder.transform([page])[0])
                            ml_prob = probs[page_idx] if page_idx < len(probs) else 0
